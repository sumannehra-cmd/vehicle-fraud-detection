...
const analyzeFraud = async (claimData, userId) => {
  ...
  // ━━ RULE 1: Gaadi blacklist mein hai? ━━━━━━━━━━━━━━━
  ...
  // ━━ RULE 2: Ek gaadi pe 6 mahine mein 2+ claims ━━━━━
  const sixMonthsAgo = new Date(Date.now() - 180 * 24 * 60 * 60 * 1000);
  const vehicleClaims = await Claim.countDocuments({
    vehicle:   claimData.vehicle,
    createdAt: { $gte: sixMonthsAgo },
  });
  if (vehicleClaims >= 2) {
    score += 35;
    flags.push(`Is gaadi pe 6 mahine mein ${vehicleClaims} claims aaye`);
  }

  // ━━ RULE 3: Ek user ne 3 mahine mein 2+ claims ━━━━━━
  ...
  if (userClaims >= 2) {
    score += 25;
    flags.push(`Is user ne 3 mahine mein ${userClaims} claims kiye`);
  }

  // ━━ RULE 4: Policy ke 30 din ke andar claim ━━━━━━━━━
  if (claimData.policy) {
    ...
    if (policy) {
      ...
      if (daysDiff < 30) {
        score += 30;
        flags.push(`Policy ke sirf ${daysDiff} din baad incident hua`);
      }
      // Policy expire ho chuki thi?
  ...
  if (claimData.claimAmount > 500000) {
    score += 20;
    flags.push(`Bahut zyada amount: ₹${claimData.claimAmount.toLocaleString('en-IN')}`);
  }

  // ━━ RULE 6: Incident 90 din purana ━━━━━━━━━━━━━━━━━━
  const incidentAge = Math.floor(
    (Date.now() - new Date(claimData.incidentDate))
    / (1000 * 60 * 60 * 24)
  );
  if (incidentAge > 90) {
    score += 15;
  ...
  if (incidentAge < 0) {
    score += 40;
    flags.push('Incident date future mein hai — impossible');
  }

  // ━━ RULE 8: Same vehicle + same date duplicate ━━━━━━
  ...
  if (duplicate) {
    score += 60;
    flags.push('Duplicate claim — same vehicle same date pe pehle bhi claim hai');
  }

  // Score 100 se zyada nahi jaayega
